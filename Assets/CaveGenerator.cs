...
public class CaveGenerator : MonoBehaviour
{
    ...
    [SerializeField]
    int2 neighboringRoomCountRange = new int2(1,5);

    [SerializeField]
    float2 neighboringRoomDistanceRange = new float2(10,20);

    [SerializeField]
    float minDistanceBetweenRooms = 6;

    [SerializeField]
    float perlinSize = 10f;
    ...
    ConcurrentDictionary<Chunk,bool> loadedChunks = new ConcurrentDictionary<Chunk,bool>();

    List<int3> generatedRoomPositions = new List<int3>();

    List<RegionCheck> RegionChecks = new List<RegionCheck>();


    ReaderWriterLockSlim regionLock = new ReaderWriterLockSlim();
    ...
    void AddRegionCheck(int3 chunkPos, int3 bl, int3 tr, Func<List<Chunk>, Task> callback)
    {
        var bottomLeft = chunkPos - bl;
        var topRight = chunkPos + tr;
    ...
    void CheckRegionAddition(Chunk newChunk)
    {
    ...
    void CheckRegionRemove(Chunk newChunk)
    {
    ...
    async Task GenerateRoom(Chunk chunk, float perlinValue)
    {
        ...
        AddRegionCheck(chunk.Position, new int3(2), new int3(1), async chunks =>
        {
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                var perlinValue = RandomFloatGenerator(chunk.Position, map.WorldSeed);
                if (perlinValue >= perlinThreshold)
                {
            ...
            while (true)
            {
                var randomDest = destinations[LimitToRange(RandomIntGenerator(chunk.Position, destCount),0,destinations.Count)];
                //tasks.AddRange();
    ...
    void DrawTunnel(int3 sourceChunk, int3 destinationChunk, int steps)
    {
        ...
        for (float i = 0; i <= steps; i++)
        {
            ...
            var brushSize = RandomFloat3Generator(sourceChunk + destinationChunk,(int)i);
    ...
    async Task DrawRoom(float3 position, List<Chunk> chunks = null)
    {
        await map.LoadChunksInArea(position,new int3(1),keepChunksLoaded: chunks != null,out_chunksInArea: chunks);
        var size = GenerateLimitedInt3(position);
        map.UseSphereBrush(position, true, 1f, size);
    }

    public float RandomFloatGenerator(float3 position, int seedOffset = 0) => RandomFloatGenerator((int3)position, seedOffset);
    ...
    public float3 RandomFloat3Generator(float3 position, int seedOffset = 0) => RandomFloat3Generator((int3)position, seedOffset);
    ...
    public int RandomIntGenerator(float3 position, int seedOffset = 0) => RandomIntGenerator((int3)position, seedOffset);
    ...
    public int3 RandomInt3Generator(float3 position, int seedOffset = 0) => RandomInt3Generator((int3)position, seedOffset);
    ...
    public int3 GenerateLimitedInt3(float3 position, int seedOffset = 0) => GenerateLimitedInt3((int3)position, seedOffset);
